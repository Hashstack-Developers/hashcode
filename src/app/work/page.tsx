...
const filters = ["All", "Web", "Mobile", "AI", "SaaS"] as const;
  ...
  return (
    <section ref={root} className="px-5 pb-24 pt-32 md:px-8 md:pb-32 md:pt-40">
      <div className="mx-auto max-w-7xl">
        <SectionHeading
          eyebrow="Portfolio"
          title="Filterable work built for demos and deal rooms."
          description="Preview modals with live metrics and rich media placeholders."
          className="mb-10"
        />
        ...
        <div className="work-grid grid gap-5 md:grid-cols-2">
          {list.map((project) => (
            <button
              key={project.id}
              type="button"
              onClick={() => setActive(project.id)}
              className="work-card text-left"
            >
              <GlassCard className={cn("min-h-[280px] overflow-hidden bg-gradient-to-br p-7", project.gradient)}>
                ...
                <div className="mt-24">
                  ...
                  <h3 className="mt-2 font-[family-name:var(--font-display)] text-3xl text-cream">
                    {project.title}
                  </h3>
      ...
      {selected && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 p-5 backdrop-blur-md">
          ...
          />
          <div className="relative z-10 w-full max-w-2xl overflow-hidden rounded-3xl border border-cream/10 bg-[#101218] p-8 shadow-2xl">
